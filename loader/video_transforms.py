...
def train_transform(img_crop_size=224, _divide=True):

    transforms_list = [
        Permute([3, 0, 1, 2]),    # T H W C -> C T H W
        transforms.RandomResizedCrop(img_crop_size, scale=(0.5, 1.0)),
        transforms.RandomHorizontalFlip(p=0.5),
    ]
    if _divide:
        transforms_list.append(DivideBy255())
    transforms_list.append(transforms_video.NormalizeVideo(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]))
    trans = transforms.Compose(transforms_list)

    return trans

def val_transform(img_crop_size=224, num_crops=1, clip_length=8, _divide=True):
...
def video_transform(split, img_crop_size=224, num_crops=1, clip_length=8):

    trans = {
        "train" : train_transform(img_crop_size),
        "test" : val_transform(img_crop_size, num_crops, clip_length),
    }

    try:
        return trans[split]
    except:
        raise BaseException("Transforms for split {} not available, Choose [train/test]".format(split))